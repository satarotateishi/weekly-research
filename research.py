...
def research_topic(topic: str) -> str:
    ...
    today_str = f"{date.today():%Y年%m月}"
    prompt = f"""あなたは中小企業診断士の視点を持つマーケティング分析者です。
現在は{today_str}です。次のテーマについてWeb検索し、最新の事例や施策を調べてください。

テーマ：{topic}

【重要】調査の方針：
- できるだけ直近6か月以内（{today_str}に近い時期）の新しい事例を優先してください。
- 何年も前の古い定番事例ではなく、最近の取り組みや今年の動向を中心に取り上げてください。
- 検索する際は「{date.today():%Y}」など時期を絞るキーワードも活用してください。

調べた内容を踏まえ、以下の構成で日本語のレポートを書いてください。
出力はそのままメール本文に使うので、前置きや「承知しました」等は不要です。

【テーマ】{topic}
■ 見つかった施策・事例（2〜4個、出典の媒体名と分かれば時期を添える）
■ なぜ効果が出たと考えられるか（簡潔に）
